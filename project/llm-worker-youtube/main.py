...
3. `__TIMELINE` 규칙
- 절대 초 단위([123.45], [75.6], [438])로 출력하지 마세요.
- 반드시 [분:초] 형태로만 출력하고, 숫자 뒤에는 한글 또는 공백이 반드시 붙어야 합니다.
- 타임라인은 5개 이상 10개 이하로 자막을 참고하여 영상 전체 구간을 시간대별로 고르게 나눠서 중요한 장면을 요약하세요.
- 반드시 타임라인은 100자 이내로 작성하되 맥락과 의도 또는 전달하려는 주제를 추론하여 단순 나열이 아닌 내용 흐름에 따른 구조화된 설명으로 작성해주세요.
- 반드시 각 타임라인 설명은 마치 사용자와 영상을 함께 보고 있는 것 처럼 부드럽고 말하듯 자연스럽게 작성하세요.
- 예를 들어: "~을 소개하고 있네요", "~에 대한 이야기가 나오고 있어요", "~에 대해 살펴보고 있어요" 등
- 반드시 딱딱한 설명문, 리포트 스타일 표현은 피하고, 친근하고 회화적인 말투로 표현하세요.
- 반드시 자막에 기반하여 실제 등장한 시간대만 포함하세요.
- 반드시 잘못된 미래 시간(예: [13844], [12652423:36] 등)은 절대 생성하지 마세요.
- 오타/잘못된 시간 표현이 있으면 즉시 실패
- 영어, 특수문자, 이상한 괄호 사용 금지
- 타임라인마다 반드시 줄바꿈하세요.  즉, [00:12], [01:23] 등 '[' 문자가 나올 때마다 줄바꿈(새 줄) 되도록 작성하고 중복되는 내용이 없도록 출력하세요.
...
4. 프롬프트 응답 포맷 예시:
 ...
 [01:42] 핵심 개념 A에 대한 정의와 간단한 시각적 예시를 보여주네요.  
 [04:10] 실제 사례를 바탕으로 개념 A 적용 과정 설명을 하고 있어요.
 [06:25] 개념 B 도입과 A와의 비교 분석 중이네요!  
 [09:00] 종합 요약 및 시청자에게 던지는 사색적 질문을 하고 있습니다.     
